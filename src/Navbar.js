import * as React from 'react';
import AppBar from '@mui/material/AppBar';
import Box from '@mui/material/Box';
import Toolbar from '@mui/material/Toolbar';
import Typography from '@mui/material/Typography';
import Button from '@mui/material/Button';
import { useNavigate } from "react-router-dom";
export default function Navbar() {
    const navigate = useNavigate();
  return (
    <Box sx={{ flexGrow: 1 }}>
  
      <AppBar position="fixed" style={{backgroundColor:'#4338ca'}}>
        <Toolbar className='d-flex justify-content-between px-5'>
          <div>
          <Typography variant="h6" component="div" sx={{ flexGrow: 1 }} onClick={()=> navigate('/')}>
            CvMaker
          </Typography>
          </div>
          <div>
          <Button color="inherit" id="navbarsignup" onClick={()=> navigate('/signup')}>SignUp</Button>
          <Button color="inherit" id="navbarlogin" onClick={()=> navigate('/login')}>Login</Button>
          <Button color="inherit" id="cvDummy" onClick={()=> navigate('/cvDemi')}>Dummy</Button>
          <Button color="inherit" id="portfolio" onClick={()=> navigate('/portfolio')}>Portfolio</Button>
          <Button color="inherit" id="about" onClick={()=> navigate('/aboutus')}>About Us</Button>
          <Button color="inherit" id="Contact" onClick={()=> navigate('/contactus')}>Contact Us</Button>
          </div>        
        </Toolbar>
      </AppBar>
    </Box>
  );
}
